  ...
  <div id="category">
    ...
    <div v-if="Itemlist.length">
      ...
      <Card class="detail"
        ...
        <span class="lablecard"
            style="background:purple"># {{ item.lable }}</span>
        ...
        <p>
          <Tooltip placement="top"
              content="点击跳转到详情页查看">
            <Button type="primary"
                @click.native="Godetail(item.article_id)">阅读全文
            </Button>
          </Tooltip>
        </p>
      </Card>
...
export default {
  ...
  methods: {
    getlableInfo(lable) {
      this.$Spin.show();
      HttpRequest('/note/getlableInfo', {lable: lable}, 'post')
          .then(res => {
            if (res.data.err == 0) {
